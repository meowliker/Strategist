...
async function main() {
  ...
  await writeFile(path.join(process.cwd(), 'data', 'snapshot.json'), JSON.stringify(snapshot, null, 2))

  const t = snapshot.totals
  console.log(`  ${t.tasks} tasks · ${t.winners} winners · ${t.losers} losers`)
  console.log(`  ${t.analysed} tasks analysed · ${t.mismatches} field mismatches`)
  console.log(`  ${formats.length} formats · ${keywords.length} keywords · ${trust.length} trust rows`)
